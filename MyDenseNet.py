...
class MyDenseNet(nn.Module):
    ...
    def __init__(
        ...
        self.fcfeatures = nn.Linear(256, 1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
            features = self.features(x[:,:,:,:,0:96])
            features = self.class_layers(features)
            features2 = self.features(x[:,:,:,:,12:76])
            features2 = self.class_layers(features2)
            features = torch.cat((features, features2), dim=1)
            features = self.fcfeatures(features)
            return features
